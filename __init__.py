from textTranslate import main as translate
from yoo import language_name

langs = ['af', 'sw', 'ja', 'el', 'nl']


#  You need to split your document into parts because of the 10K char max limit
txt1 = """
...
def transDoc():
    try:
        for text in [txt1, txt2, txt3]:
            # for lang in langs:
            translated1 = translate(text, langs)
            for lang in langs:
                with open(f'./readme/README_{lang.upper()}.md', 'a', encoding="utf-8") as f:
                    f.write(translated1[lang])
                print(f"{lang}: {language_name(lang)}")
